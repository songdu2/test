...
if ($w == '') {
    if ($wr_id) {
        alert('글쓰기에는 \$  값을 사용하지 않습니다.', G5_BBS_URL.'/board.php?bo_table='.$bo_table);
    }
...
// 메일발송기능 사용 유무 체크(내글에 다른 이용자가 답변을 달았을시 메일을 받음)
// 기본환경설정에서 메일발송기능을 사용해야하고, 게시판 옵션에도 메일발송기능을 사용해야함
$is_mail = false;
if ($config['cf_email_use'] && $board['bo_use_email'])
    $is_mail = true;

// 작성모드일때나 수정모드일때 답변메일받기에 체크해놨음 답변메일받기 디폴트 체크
$recv_email_checked = '';
if ($w == '' || strstr($write['wr_option'], 'mail'))
    $recv_email_checked = 'checked';

// 게스트(비로그인)가 작성,수정할때나 관리자가 수정할때만(작성자와 로그인유저가 다름)
// 이름, 비번, 이메일, 홈피 입력영역이 활성화됨
$is_name     = false;
$is_password = false;
$is_email    = false;
$is_homepage = false;
