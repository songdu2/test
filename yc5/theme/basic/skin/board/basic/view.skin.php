...
<script>
	...
	function fnTestFile(mode){
		  ...
		  onrendered: function(canvas) {
			...
			document.getElementById("testForm").action = '/theme/basic/skin/board/basic/'+mode+'_test.php';
	...
	function fnTestMail() {	
		if(confirm("<?php echo $view['wr_name']; ?>"+'님('+"<?php echo $view['wr_email']; ?>"+')께 메일을 발송하시겠습니까?')){
			<?php 
			
			if ($config['cf_email_use'] && $board['bo_use_email']) {

				$title = '['.$config['cf_title'].'] '.$board['bo_subject'].' 에서 알람을 보냈습니다~^^';

				$wr_subject = get_text(stripslashes($view['wr_subject']));
				$wr_name = get_text(stripslashes($view['wr_name']));
				$wr_content = conv_content(conv_unescape_nl(stripslashes($view['wr_content'])), 0);
				...
				//mailer2('master', 'rkeh001@gmail.com', 'jewy@naver.com', $title, $content, 1);
			}
...
<form name="testForm" id="testForm" method="post" target="_blank">
	<input type="hidden" id="imgUrl" name="imgUrl">
</form>

<button class="btn_b02 btn" onclick="fnTestFile('img'); return false;">IMG</button>
<button class="btn_b02 btn" onclick="fnTestFile('pdf'); return false;">PDF</button>
<button class="btn_b02 btn" onclick="fnTestMail(); return false;">MAIL</button>
